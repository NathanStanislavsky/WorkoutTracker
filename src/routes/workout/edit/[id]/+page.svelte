<script lang="ts">
	...
	let workoutDate = new Date(workout.date).toISOString().split("T")[0];
	let workoutDuration = workout.duration;
	let workoutCalories = workout.caloriesBurned;
  </script>
  
  <form method="post" class="max-w-md mx-auto p-6 bg-slate-800 shadow-md rounded">
	<h2 class="text-2xl mb-4 text-white">Edit Workout</h2>
	...
	<div class="mb-4">
	  <label for="workout-duration" class="block mb-2 text-white">Duration</label>
	  <input
		type="number"
		id="workout-duration"
		name="duration"
	...
	<div class="mb-4">
	  <label for="calories-burned" class="block mb-2 text-white">Calories Burned</label>
	  <input
		type="number"
		id="calories-burned"
		name="calories"
